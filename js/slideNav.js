...
var SlideNav = function () {
	...
	_createClass(SlideNav, [{
		...
		value: function observe() {
			...
			try {
				for (var _iterator = this.navAnchors[Symbol.iterator](), _step; 
				!(_iteratorNormalCompletion = (_step = _iterator.next()).done);
				 _iteratorNormalCompletion = true) {
					var anchor = _step.value;
					//This where it click event for tab or link is call
					anchor.addEventListener("click", function (e) {
						e.preventDefault();
						var linkHash = _this.getHash(e.currentTarget.href);
						if (!_this.goToSection(linkHash) 
						&& e.currentTarget.href) _this.goToUrl(e.currentTarget.href);
					});
				}
			} catch (err) {
		...
		value: function scrollTo(destOffset, duration) {
			...
			var diffOffset = destOffset - this.scrollDoc.scrollTop+10,
